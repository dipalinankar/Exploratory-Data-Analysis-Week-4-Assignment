...
#filter data/rows from dataset_1 where M_HUM_ is less than 29 M_NBP greather than 70.4. store it in Dataset_HUM29_NBP70.4
Dataset_HUM29_NBP70.4 <- subset(dataset_1, dataset_1$M_NBP > 70.4 & dataset_1$M_HUM_ < 29)

#check few rows from Dataset_HUM29_NBP70.4
head(Dataset_HUM29_NBP70.4)

#dross check minimum and maximum values of M_HUM_ field from filtered dataset Dataset_HUM29_NBP70.4 
max(Dataset_HUM29_NBP70.4$M_HUM_)
min(Dataset_HUM29_NBP70.4$M_HUM_)
...
for (i in 1:nrow(Dataset_HUM29_NBP70.4)) {
  # execute this loop only when genotypename is not null
  if (!is.na(Dataset_HUM29_NBP70.4$GENOTYPE_NAME[i])) {
    ...
    if(substr(Dataset_HUM29_NBP70.4$GENOTYPE_NAME[i],1,2)=="TX") { 
      Dataset_HUM29_NBP70.4$Col[i] <-  "lightgreen"
    }else if(substr(Dataset_HUM29_NBP70.4$GENOTYPE_NAME[i],1,2)=="P0") {
      ...
      Dataset_HUM29_NBP70.4$Col[i] <- "sienna1"
    }else if(substr(Dataset_HUM29_NBP70.4$GENOTYPE_NAME[i],1,2)=="CA") {
      ...
      Dataset_HUM29_NBP70.4$Col[i] <- "purple1"
    } else
      # for all other genotype names which are not satisfying above conditions, set color code to gray.
      Dataset_HUM29_NBP70.4$Col[i] <- "slategray"
    
  }
}


#to check color code values assigned in COl field in new dataset
Dataset_HUM29_NBP70.4$Col

# draw graphics using ggplot with dataset Dataset_HUM29_NBP70.4 and x axis with M_HUM_ field and y axis with M_RDT field
#to ignore the null values for these fields use argument na.rm=TRUE

ggplot(Dataset_HUM29_NBP70.4, aes(x=M_HUM_, y=M_RDT),na.rm=TRUE) +
  ...
  geom_point(aes(size=Dataset_HUM29_NBP70.4$M_HTP,color =Dataset_HUM29_NBP70.4$Col),na.rm=TRUE) +
  ...
  geom_label_repel(aes(label=Dataset_HUM29_NBP70.4$GENOTYPE_NAME),size=3,na.rm = TRUE, nudge_x = 0,nudge_y =7, box.padding = unit(0.25, "lines")) +
  # to reverse the x axis scale from range as shown in original graphs
  scale_x_reverse(breaks = seq(29, 20, by = -1), limits=c(29,20)) +
